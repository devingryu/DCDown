...
class ConPackListAdapter(private val listener: ConPackListener, val activity: Activity) :
                    ...
                    with(holder as ConPackViewHolder) {
                        (packList[position] as ConPack).let { item ->
                            ViewCompat.setTransitionName(
                                binding.card,
                                mContext.getString(R.string.main_detail_transition_name, item.idx)
                            )
                            val requestOptions =
                                RequestOptions()
                                    .dontAnimate()
    ...
    inner class ConPackViewHolder(val binding: RecyclerConpackBinding) :
        RecyclerView.ViewHolder(binding.root)

    inner class HeaderViewHolder(val binding: RecyclerHeaderBinding) :
        RecyclerView.ViewHolder(binding.root)

    inner class EndedViewHolder(val binding: RecyclerEndedBinding) :
        RecyclerView.ViewHolder(binding.root)
}